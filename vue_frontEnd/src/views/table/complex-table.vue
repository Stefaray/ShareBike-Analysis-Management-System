<template>
  <div class="app-container">
    ...
    >
      <el-table-column label="订单ID" prop="id" sortable="custom" align="center" width="100" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.orderid }}</span>
        </template>
      </el-table-column>
      <el-table-column label="单车ID" width="80px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="起始点经度" width="130px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.start_location_x }}</span>
        </template>
      </el-table-column>
      <el-table-column label="起始点纬度" width="130px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.start_location_y }}</span>
        </template>
      </el-table-column>
      
      <el-table-column label="结束点经度" width="130px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.end_location_x }}</span>
        </template>
      </el-table-column>
      <el-table-column label="结束点纬度" width="130px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.end_location_y }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="width: 400px; margin-left:50px; ">
        ...
        <el-form-item label="订单结束时间" prop="timestamp">
        ...
        <el-form-item label="途径地点">
          <el-input v-model="temp.track" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="用经纬度表示..." />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios.post(
                this.$baseURL+'order/order_table', 
                qs.stringify({
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          .then((response)=>{
                ...
                console.log(response)
                this.temp.orderid = response.data.orderid
